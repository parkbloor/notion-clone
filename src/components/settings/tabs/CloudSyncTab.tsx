...
export default function CloudSyncTab() {
  ...
  // ── 상태 메시지 렌더 헬퍼 ──────────────────────
  // Python으로 치면: def render_msg(key): return <p class={state}>{msg}</p>
  function Msg({ actionKey }: { actionKey: string }) {
  ...
  // ── 각 제공자 카드 렌더 ─────────────────────────
  // Python으로 치면: def render_provider_card(provider, icon, name): ...
        ...
        <div className="p-4 space-y-3">
          ...
          {isConnected ? (
            <>
              ...
              <Msg actionKey={`${provider}_upload`} />
              <Msg actionKey={`${provider}_download`} />
            </>
          ) : (
            <>
              {/* 미연결: 자격증명 입력 + 연결 버튼 */}
              ...
              <Msg actionKey={`${provider}_save`} />
              <Msg actionKey={`${provider}_connect`} />
