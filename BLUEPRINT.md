...
> **작성일:** 2026-02-21 | **최종 수정:** 2026-08-24 (일정 데이터 안정화 P15)
...
- P6은 기존 데이터를 가져오지 않고 빈 SQLite 저장소를 명시적으로 활성화하는 `새 일정으로 시작` 경로를 추가했다. `START_EMPTY` 확인값, 삭제 항목을 포함한 일정·회고 0건 검사, `activation_mode=fresh` 기록과 모드 변경 이벤트를 적용했으며, 기존 페이지·Day Planner 블록·루틴 파일을 읽거나 수정하거나 삭제하지 않는다. 기존 데이터가 있으면 HTTP 409으로 차단하고, 기존 안전 이전은 별도 경로로 유지한다.
- P7은 SQLite 모드의 우측 오늘 일정 패널에 제목·날짜·시간·색상 편집, 소프트 삭제 직후 실행 취소, 선택 날짜의 삭제함 복원과 최신 revision 기반 409 재적용 선택을 연결했다. 공백 제목과 역순 시간은 클라이언트·서버 모두 거부하며 기존 페이지·Day Planner 블록은 변경하지 않는다.
- P8은 SQLite 모드의 Day Planner 블록에 별도 하루 타임라인을 연결했다. 빈 슬롯 추가·날짜 이동·현재 시각선·완료 숨기기·기존 시간대/스냅/줌 설정과 데스크톱 알림을 SQLite 일정에 적용하며, 드래그·리사이즈는 로컬 미리보기 뒤 마우스를 놓을 때 한 번의 revision 저장만 한다. HTTP 409은 최신 값 재조회와 명시적 재적용으로 처리하고 기존 원본 블록은 보존 안내를 계속 표시한다.
- P9는 SQLite 스키마 v2의 `routines`·`routine_policy`·`migration_routines`로 루틴을 분리했다. 새 SQLite 루틴은 revision, 요일, 활성 상태, 자동 적용 정책을 가지며 날짜별 적용은 `routineId+date` 중복을 트랜잭션에서 막는다. 기존 `_planner_routines.json`은 검증 백업이 현재 원본과 일치하고 사용자가 명시적으로 복사를 선택했을 때만 복사하며 원본 파일을 수정·삭제하지 않는다.
- P10은 SQLite 일정 상세 창에 예정 여부·실제 기록·하위 작업·집중도와 서버 시각 타이머를 연결했다. 타이머는 서버 UTC 기준으로 재시작 뒤에도 경과 시간을 복구하고, 저장소 전체에서 한 일정만 실행하도록 409 충돌로 보호한다.
- P11은 SQLite 일일 회고를 우측 일정 패널에 연결했다. 같은 날짜의 완료·계획·실제 시간 요약과 함께 저장하며, revision 충돌은 최신 회고를 읽은 뒤 사용자 초안을 명시적으로 재적용한다.
- P12는 SQLite 모드의 주간 타임라인과 월간 일정 레이어를 추가했다. 주간은 한 번의 7일 범위 조회로 표시하고 요일 간 이동은 단일 revision 업데이트로 저장한다. 월간은 일별 일정·완료 상태를 표시하고 날짜 선택을 기존 우측 SQLite 일정 패널에 연결하며, 메모·기록 달력은 변경하지 않는다.
- P13은 `WeeklyPlannerBlock`·`RoutineMatrixBlock`·`QuarterlyPlannerBlock`·`YearlyPlannerBlock`을 공통 SQLite 범위 조회 어댑터로 연결했다. SQLite 모드에서 루틴 완료율, 계획·실제 시간, 활동일은 일정/루틴 단일 원본으로만 집계하며, 레거시 페이지 통계와 섞지 않는다. 기존 블록의 레거시 모드와 원본 데이터는 보존한다.
- P14는 일정·회고·루틴·정책을 포함한 checksum JSON 백업, 추가/중복/충돌을 먼저 확인하는 트랜잭션 복원, CSV와 SQLite 일정·회고+기존 기록 헤더의 기간 HTML, 삭제 SQLite 일정 탐색/복원을 설정 화면에 연결했다. 복원은 충돌이 없고 미리보기 지문이 현재와 같을 때만 SQLite 모드를 활성화하며, 레거시 페이지와 아카이브 원본을 수정하지 않는다.
- P15는 SQLite 타임라인에서 AI JSON을 즉시 저장하지 않고 revision 기반 추가/교체 diff로 먼저 표시하게 했다. 사용자가 명시적으로 적용하면 배치 API가 모든 revision을 검사한 뒤 하나의 트랜잭션으로 반영하며 루틴 일정은 AI 교체에서 제외한다. 날씨는 기존 위치 설정과 API의 읽기 전용 보조 배지로 유지한다.
- 일정 데이터 안정화 전체 자동 검증은 백엔드 78건, 프런트 단위 24건, TypeScript, 변경 범위 ESLint와 Next.js 16.3.2 프로덕션 빌드를 통과했다. `Test_vault` 실제 검증에서 빈 SQLite v2 활성화, 재접속, 두 클라이언트의 200/409 revision 충돌, 삭제/복원/삭제함, checksum 백업·CSV·HTML을 확인했다. 별도 임시 Chrome의 실제 네트워크 전환으로 일정 API `online → offline → online` 복구를 확인했고, SQLite 상태 API가 오프라인이면 플래너 소비자는 레거시 원본으로 폴백하지 않고 읽기 전용 상태를 표시한다.
- 일정 안정화 후속 계획은 P16의 실제 앱 회귀 검증과 레거시 소비자 감사를 남겼다. 기존 블록과 실제 사용자 데이터는 자동 삭제하거나 변환하지 않는다.
